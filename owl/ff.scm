...
(define-library (owl ff-ng)
   ...
   (begin
      ...
      (define (put ff k v)
         (if (eq? ff #empty)
            (mkt type-ff k v)
            (lets ((l this-k this-v r (explode ff)))
               (cond
                  ((lesser? k this-k)
                     (node (put l k v) this-k this-v r))
                  ((eq? k this-k)
                     (node l k v r))
                  (else
                     (node l this-k this-v
                        (put r k v)))))))


      (let ((ff (fold (λ (ff i) (put ff i i)) #empty (list 1 2 3 4 5 6 7 8 9))))
         (if (eq? 5 (get ff 5 'bad))
            (if (eq? 'no (get ff 0 'no))
               'ok
               (car 'badd)) ;; no error handling here yet
            (car 'bad)))
))
