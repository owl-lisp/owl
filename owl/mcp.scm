...
(define-library (owl mcp)
   ...
   (import
      ...
      (scheme misc)
      (only (owl math-extra) !) ; <- ?
      (only (owl env) mcp-tag link-tag buffer-tag poll-tag) 
      (only (owl dump) dump-fasl)
      (only (owl io) start-base-threads)
         ...
         (let loop ((threads (map cdr threads)) (vm-threads null) (state state))
            (if (null? threads)
               (begin
                  ...
                  (if maybe-cont
                     (maybe-cont maybe-cont vm-threads null state) ; start running the given controller 
